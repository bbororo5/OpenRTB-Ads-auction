...
다음 구현 전에 세 기술 결정을 검증해야 한다.

1. 리전 예산 원장이 어떤 DBMS로 리스 발급·정산의 단일 권위를 보존할 것인가.
2. DSP 인스턴스가 로컬 리스와 5초 예약을 어떤 자료구조로 처리할 것인가.
3. 인스턴스 장애를 상위 리스 격리로 복구할 수 있는데도 로컬 DBMS가 필요한가.

두 번째와 세 번째는 독립 선택이 아니다. 먼저 로컬 내구성이 필요한지 결정하고, 필요하지 않으면 메모리 자료구조를 선택한다. 필요할 때만 내장 DBMS 후보를 비교한다.
...
| 로컬 리스 권한 | 인스턴스에 위임된 금액 권위 | 캠페인별 조건부 차감, 세대·만료와 잔액을 함께 보호 | 캠페인 인덱스→`LeaseAccount`, 조건부 CAS 또는 샤드 단일 작성자 | 매 입찰 경로에는 사용하지 않음 |
| 5초 예약 | 리스 권한의 잠재 지출 상태 | `lurl`·`burl`·만료 중 한 번만 종결, 높은 생성·삭제 빈도 | 예약 ID 해시 인덱스, 원자 상태 전이, 만료 버킷·타이밍 휠 | 과금 사건만 외부에 내구 기록 |
...
단순 합계 카운터만으로는 부족하다. `잔액 충분 여부 확인`과 `차감`이 한 연산이어야 하고, 리스 세대·활성 상태·기한이 바뀌는 순간의 예약도 함께 차단해야 한다.

```text
LeaseAccount
  ├─ leaseId·generation·expiresAt
  ├─ ACTIVE | CLOSED
  └─ remainingMicros
```

잔액만 `AtomicLong`으로 두면 리스 종료와 차감 사이의 경쟁을 별도로 막아야 한다. 상태와 잔액을 하나의 원자 상태로 표현하거나 캠페인 샤드의 단일 작성자가 함께 변경해야 한다. `LongAdder`는 정확한 조건부 차감을 제공하지 않으므로 사용할 수 없다.
...
종결 상태는 CAS로 한 번만 바꾼다. 해제에 성공한 실행자만 리스 가용액을 반환한다. 만료 탐색은 전체 예약 맵을 순회하지 않고 시간 버킷이나 타이밍 휠에서 대상 ID만 가져온다.
...
첫 비교 기준선은 트랜잭션 RDBMS다. 리스 쓰기 부하와 인기 캠페인 경합이 실제 한계를 만들 때 내구형 인메모리 DBMS와 분산 SQL을 같은 시험으로 비교한다.

## 3. DSP 로컬 리스 토큰 자료구조

### 상태

로컬 상태는 하나의 토큰 객체가 아니라 두 조회 경계로 나뉜다.

```text
campaignId → 활성 리스 세대·잔액·기한·페이싱 위치
reservationId → 캠페인·리스·금액·5초 기한·종결 상태
```

예약 만료를 찾기 위한 시간 인덱스도 필요하다.

```text
만료 버킷 또는 타이밍 휠 → reservationId
```
...
| 후보 | 강점 | 감수할 점 |
|---|---|---|
| 동시 해시 인덱스 + 캠페인별 CAS | 가장 단순한 기준선이며 캠페인별 조건부 차감을 직접 표현한다. | 인기 캠페인의 같은 원자 변수에 경합이 집중될 수 있다. |
...
초기 기준선은 `campaignId` 해시 인덱스, 캠페인별 조건부 CAS, `reservationId` 해시 인덱스와 만료 버킷·타이밍 휠의 조합이다. `LongAdder`는 합계 조회에는 유용하지만 “잔액이 충분할 때만 정확히 차감”하는 연산을 하나로 보장하지 못하므로 예약 권위에는 사용하지 않는다.
...
| 후보 | 강점 | 감수할 점 | 현재 판단 |
|---|---|---|---|
| DBMS 없이 프로세스 메모리 사용 | 0-Hop, 가장 낮은 지연과 단순한 장애 경계 | 장애 리스를 `leaseExpiresAt + 5초`까지 격리한 뒤 외부 원본으로 재구성 | 우선 기준선 |
