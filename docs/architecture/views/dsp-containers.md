...
C4Container
    ...
    Container_Boundary(project_dsp, "프로젝트 DSP") {
        Container(gateway, "DSP 게이트웨이", "기술 미정", "입찰 소유자를 안정적으로 선택하고 통지를 전달하며 경로별 과부하를 조기 차단한다.")
...
|---|---|---|
| DSP 게이트웨이 | `sspId + BidRequest.id` 기반 입찰 소유자 라우팅, 통지 경로 분류와 과부하 차단 | 라우팅·상태 확인·용량 예약 구현 |
