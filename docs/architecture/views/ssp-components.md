...
|---|---|---|---|
| 공급자 시스템 | 프로젝트 공급자 경매 요청 | 낙찰 결과·렌더링 증표 또는 no-bid | `POST application/json`, 64KiB 이하만 받는다. 지역 설정 스냅숏의 공급자 ID·키 ID·활성 상태를 검증하며 같은 요청은 최초 결과를 재사용한다. 내용 충돌 `409`, 포화 `503`, 절대 기한 초과 `504`를 구분한다. |
| 광고 표시 클라이언트 | 발급 리전 URL의 `RenderCompleted(renderProof)` | `204 No Content` | `POST application/json`, 8KiB 이하만 받는다. `204`는 청구·전달 작업 커밋 뒤에만 반환한다. 같은 증표는 다시 `204`, 잘못된 증표는 `400`, 저장 일시 실패는 `503`이다. |
