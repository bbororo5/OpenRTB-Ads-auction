...
같은 SSP 요청의 최초 호출 하나만 캠페인 선택과 예약을 실행한다. 후속 호출은 최초 결과를 기다리거나 재사용하지 않고 빠르게 거절한다. 기존 실행의 소유권이 불확실해도 새 예약을 만들지 않는다. 반환한 각 입찰에는 정확히 하나의 유효한 예약이 있어야 한다. 페이싱 지연이 큰 캠페인부터 예약하고, 동시 요청과 경합해 예약하지 못하면 같은 절대 기한 안에서 다음 캠페인을 시도한다. 한 슬롯의 실패는 다른 슬롯의 성공을 취소하지 않는다.
