...
상태: 얇은 수직 흐름 0~5단계 완료 · 컴포넌트 강화 1~8단계 완료 · 독립 운영 검증 완료
...
SSP만 소유하는 운영 경계는 구현·검증했다.

| 경계 | 보장 | 검증 |
|---|---|---|
| 시작 | 공급자 설정 스냅숏과 청구 저장소를 검증하지 못하면 HTTP 포트를 열지 않는다 | 실제 PostgreSQL 통합시험 |
| 상태 | `/health/live`는 프로세스 생존, `/health/ready`는 신규 업무 요청 수락 가능 상태만 반환한다 | HTTP 단위시험·두 지역 런타임 확인 |
| 종료 | 종료 중 새 경매·렌더링 요청은 `503 SERVER_DRAINING`으로 거절하고, 이미 수락한 요청은 최대 1초 동안 마무리한다 | HTTP 단위시험 |
| 관측 | `/metrics`는 경매·렌더링 응답 상태, 진입 거절 사유, 처리 중 요청 수를 Prometheus 형식으로 노출한다 | HTTP 단위시험·두 지역 런타임 확인 |
| 지역 실행 | 로컬 Compose가 서울·도쿄 SSP를 각각 자기 PostgreSQL에 연결한다. DSP 주소는 설정만 하고 호출하지 않는다 | `verify-ssp-runtime.sh` |

실제 경매 성공, 세 DSP 경쟁, `nurl`·`lurl`·`burl` 시스템 간 전달과 성능·장애 시험은 프로젝트 DSP 구현 후의 통합 범위다.
