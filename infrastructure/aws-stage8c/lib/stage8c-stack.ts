...
export class Stage8cStack extends Stack {
  ...
  private configureDsp(
    ...
    userData.addCommands(
      ...
      `docker run -d --name dsp --restart unless-stopped --network host -v /opt/rtb/campaigns.json:/opt/rtb/campaigns.json:ro ${this.otelJavaEnvironment("rtb-dsp")} -e JAVA_TOOL_OPTIONS='-Xms768m -Xmx768m -XX:+UseG1GC -javaagent:/otel/opentelemetry-javaagent.jar' -e SERVER_PORT=8081 -e DSP_REGION_ID=${this.region} -e DSP_INSTANCE_ID=dsp-stage8c-1 -e DSP_PUBLIC_BASE_URL=http://${supportPrivateIp}:8080 -e DSP_CAMPAIGN_SNAPSHOT_PATH=/opt/rtb/campaigns.json -e DSP_CAMPAIGN_VERSION=v1 -e DSP_CAMPAIGN_SHA256=\"$CAMPAIGN_SHA\" -e DSP_NOTICE_TOKEN_ACTIVE_KEY_ID=key-1 -e DSP_NOTICE_TOKEN_KEYS=key-1=\"$NOTICE_KEY\" -e DSP_LEDGER_JDBC_URL=jdbc:postgresql://${supportPrivateIp}:5433/rtb -e DSP_OUTCOME_JDBC_URL=jdbc:postgresql://${supportPrivateIp}:5434/rtb -e DSP_STORE_USERNAME=postgres -e DSP_STORE_PASSWORD='${databasePassword}' -e DSP_MINIMUM_LEASE_MICROS=100000 -e DSP_BID_WORKERS=8 -e DSP_NOTICE_WORKERS=4 -e DSP_LEDGER_POOL_SIZE=6 -e DSP_OUTCOME_POOL_SIZE=6 -e DSP_JDBC_WORKERS=6 ${dspImage}`,
    );
  }
