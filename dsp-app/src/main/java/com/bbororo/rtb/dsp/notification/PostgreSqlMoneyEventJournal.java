...
public final class PostgreSqlMoneyEventJournal implements MoneyEventJournal {
    ...
    public PostgreSqlMoneyEventJournal(DataSource dataSource, Executor jdbcExecutor) {
        this.dataSource = Objects.requireNonNull(dataSource, "dataSource");
        this.jdbcExecutor = Objects.requireNonNull(jdbcExecutor, "jdbcExecutor");
    }

    @Override
    public CompletionStage<EventAppendResult> append(MonetaryNoticeEvent event) {
        Objects.requireNonNull(event, "event");
        return CompletableFuture.supplyAsync(() -> appendBlocking(event), jdbcExecutor);
    }

    private EventAppendResult appendBlocking(MonetaryNoticeEvent event) {
            ...
            try {
                ...
                if (inserted == 0) {
                    ...
                    if (existing != null && existing.matches(event)) {
                        connection.commit();
                        return new EventAlreadyPresent(event.eventId());
                    }
                    recordConflict(connection, event, existing);
                    connection.commit();
                    return new EventConflict(event.eventId(), existingKind(existing, event));
                }

                insertOutcome(connection, event);
                StoredOutcome outcome = findOutcome(connection, event.reservationId());
                if (outcome.eventId().equals(event.eventId())) {
                    connection.commit();
                    return new EventAppended(event.eventId());
                }
                ...
                connection.commit();
                return new EventConflict(event.eventId(), outcome.kind());
    ...
    private void insertOutcome(Connection connection, MonetaryNoticeEvent event) throws SQLException {
    ...
    private record StoredOutcome(
        ...
        StoredEvent asEvent() {
            return new StoredEvent(eventId, kind, "", leaseId, campaignId, amountMicros, expiresAt);
        }
    }
