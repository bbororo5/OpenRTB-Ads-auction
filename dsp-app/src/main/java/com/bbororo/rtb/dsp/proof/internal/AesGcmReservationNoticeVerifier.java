...
import com.bbororo.rtb.dsp.proof.internal.NoticeTokenEnvelope.ParsedEnvelope;
...
public final class AesGcmReservationNoticeVerifier implements ReservationNoticeVerifier {
    ...
    public NoticeVerification verify(NoticeToken token) {
        ...
        if (!claims.authenticatedSspId().equals(token.authenticatedSspId())) {
            return new InvalidReservationNotice(InvalidNoticeReason.WRONG_SSP);
        }
